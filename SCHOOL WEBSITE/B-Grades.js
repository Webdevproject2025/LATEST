function loadGradesData() {
    ...
    const studentGrades = [
        { no: 1, name: 'Student One', gender: 'Male', readingWriting: '', Pagbasa: '', ucsp: '', statistic: '', pe: '', pr1: '', englishforacademic: '', foodbeverageQ1: '', foodbeverageQ2: '' },
        { no: 2, name: 'Student Two', gender: 'Female', readingWriting: '', Pagbasa: '', ucsp: '', statistic: '', pe: '', pr1: '', englishforacademic: '', foodbeverageQ1: '', foodbeverageQ2: '' },
    ...
    if (gradesTableBody && studentGrades) {
        studentGrades.forEach(student => {
            ...
            row.insertCell().textContent = student.readingWriting;
            row.insertCell().textContent = student.Pagbasa;
            row.insertCell().textContent = student.ucsp;
            row.insertCell().textContent = student.statistic;
            row.insertCell().textContent = student.pe;
            row.insertCell().textContent = student.pr1;
            row.insertCell().textContent = student.englishforacademic;
            row.insertCell().textContent = student.foodbeverageQ1;
            row.insertCell().textContent = student.foodbeverageQ2;
        });
    } else {
...
function addNewStudentRow() {
    ...
    newRow.insertCell().innerHTML = '<input type="number" class="grade-input food-beverage-q1" min="0" max="100" oninput="calculateRowAverage(this.closest(\'tr\'))">';
    newRow.insertCell().innerHTML = '<input type="number" class="grade-input food-beverage-q2" min="0" max="100" oninput="calculateRowAverage(this.closest(\'tr\'))">';
...
function saveGradesData() {
    ...
    for (let i = 0; i < tableRows.length; i++) {
        ...
        rowData.readingWriting = gradeInputs[0]?.value;
        rowData.Pagbasa = gradeInputs[1]?.value;
        rowData.ucsp = gradeInputs[2]?.value;
        rowData.statistic = gradeInputs[3]?.value;
        rowData.pe = gradeInputs[4]?.value;
        rowData.pr1 = gradeInputs[5]?.value;
        rowData.englishforacademic = gradeInputs[6]?.value;
        rowData.foodbeverageQ1 = tableRows[7]?.value; 
        rowData.foodbeverageQ2 = tableRows[8]?.value; 
